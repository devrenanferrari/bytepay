...
// Função que irá buscar os temas e gravar o ID do tema no banco de dados
    ...
    .then(responseData => {
      ...
      themes.forEach(theme => {
        ...
        if (theme.node.name === 'sabino-nichado') {
          const themeId = theme.node.id.split('/').pop(); // Extraindo o ID

          // Gravar o ID do tema no banco de dados
...
// Função para salvar o ID do tema no banco de dados
function saveThemeIdToDatabase(themeId) {
  ...
  const userId = 1;  // Substitua pelo ID real do usuário, se necessário
